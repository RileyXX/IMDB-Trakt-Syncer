...
def prompt_get_credentials():
    ...
    values = {
        key: file_data.get(key, default_value)
        for key, default_value in default_values.items()
    }

    # Prompt user for missing credentials, excluding tokens
    for key, value in values.items():
        if value == "empty" and key not in [
            "trakt_access_token",
            "trakt_refresh_token",
            "last_trakt_token_refresh",
        ]:
            if key == "imdb_username":
                prompt_message = (
                    f"Please enter a value for {key} (email or phone number): "
                )
                ...
                print(
                    "  1. Login to Trakt and navigate to your API apps page: "
    ...
    if last_trakt_token_refresh != "empty":
        try:
            last_trakt_token_refresh_time = datetime.datetime.fromisoformat(
                last_trakt_token_refresh
            )
            if datetime.datetime.now() - last_trakt_token_refresh_time < timedelta(
                days=7
            ):
    ...
    if should_refresh:
        ...
        if "trakt_refresh_token" in values and values["trakt_refresh_token"] != "empty":
            ...
            trakt_access_token, trakt_refresh_token = authTrakt.authenticate(
                client_id, client_secret, trakt_access_token
            )
        else:
            trakt_access_token, trakt_refresh_token = authTrakt.authenticate(
    ...
    return (
        ...
        values["trakt_access_token"],
        values["trakt_refresh_token"],
        values["imdb_username"],
        values["imdb_password"],
    )


def prompt_sync_ratings():
    """
    Prompts the user to enable or disable syncing of ratings and updates the credentials
    file accordingly.
    """
    # Define the file path
    here = os.path.abspath(os.path.dirname(__file__))
    ...
    try:
        ...
        # Log the error if the file is missing, continue with an empty credentials
        # dictionary
...
def prompt_sync_watchlist():
    """
    Prompts the user to sync their watchlist if not already configured in
    credentials.txt.
...
def prompt_sync_watch_history():
    ...
    while True:
        ...
        print(
            "See FAQ: "
            "https://help.imdb.com/article/imdb/track-movies-tv/check-ins-faq/"
            "GG59ELYW45FMC7J3"
        )
        user_input = (
            input("Do you want to sync your watch history? (y/n): ").strip().lower()
        )
        if user_input == "y":
...
def prompt_mark_rated_as_watched():
    ...
    while True:
        user_input = (
            input("Do you want to mark rated movies and episodes as watched? (y/n): ")
            .strip()
            .lower()
        )
        if user_input == "y":
...
def check_imdb_reviews_last_submitted():
    ...
    file_path = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "credentials.txt"
    )
    ...
    if last_submitted_str:
        try:
            last_submitted_date = datetime.datetime.strptime(
                last_submitted_str, "%Y-%m-%d %H:%M:%S"
            )
        except ValueError:
    ...
    if datetime.datetime.now() - last_submitted_date >= datetime.timedelta(hours=240):
        ...
        credentials["imdb_reviews_last_submitted_date"] = (
...
def prompt_sync_reviews():
    """
    Handles the synchronization preference for reviews by reading and updating a
    credentials file.
    Returns:
    ...
    while True:
        print(
            "Please note: reviews synced to IMDB will use 'My Review' as the title "
            "field."
        )
...
def prompt_remove_watched_from_watchlists():
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as file:
            credentials = json.load(file)
            remove_watched_from_watchlists_value = credentials.get(
                "remove_watched_from_watchlists"
            )
            if (
                remove_watched_from_watchlists_value is not None
                and remove_watched_from_watchlists_value != "empty"
            ):
    ...
    while True:
        ...
        print(
            "Shows are removed when at least 80% of the episodes are watched AND the "
            "series is marked as ended or cancelled."
        )
...
def prompt_remove_watchlist_items_older_than_x_days():
    """
    Prompts the user to decide if watchlist items older than a certain number of days
    should be removed.
    Reads and updates the decision and the number of days in a credentials file to avoid
    repeated prompting.
    """
    # Define the file path for credentials
    here = os.path.abspath(os.path.dirname(__file__))
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as file:
            credentials = json.load(file)
            remove_watchlist_items_older_than_x_days = credentials.get(
            ...
            if remove_watchlist_items_older_than_x_days is not None and (
                not remove_watchlist_items_older_than_x_days
                or watchlist_days_to_remove is not None
            ):
                return (
                    remove_watchlist_items_older_than_x_days,
    ...
    while True:
        print(
            "If choosing (y) in the following, you will be prompted to enter the number"
            " of days."
        )
        print(
            "This setting is meant to help address the 100 item limit in Trakt "
            "watchlists for free tier users."
        )
        print(
            "In order to prevent old items from being re-added, it is recommended to "
            "disable Trakt watchlist sync"
        )
        print(
            "in other projects when enabling this setting. Such as Reelgood and other "
            "similar apps."
        )
        print(
            "If you use the PlexTraktSync project, it is recommended to disable "
            "watchlist sync from Plex to Trakt."
        )
        print(
            "Do you want to remove watchlist items older than x number of days? (y/n)"
        )
        user_input = input("Enter your choice: ").strip().lower()
    ...
    if remove_watchlist_items_older_than_x_days:
        while True:
            try:
                print(
                    "For reference: (30 = 1 month, 90 = 3 months, 180 = 6 months, 365 ="
                    " 1 year). Any number of days is valid."
                )
                print("How many days old should the items be to be removed?")
                watchlist_days_to_remove = int(
                    input("Enter the number of days: ").strip()
                )
                break
            except ValueError:
    ...
    credentials["remove_watchlist_items_older_than_x_days"] = (
        remove_watchlist_items_older_than_x_days
    )
    credentials["watchlist_days_to_remove"] = (
        watchlist_days_to_remove if remove_watchlist_items_older_than_x_days else None
    )
    try:
        with open(file_path, "w", encoding="utf-8") as file:
            json.dump(credentials, file, indent=4, separators=(", ", ": "))
