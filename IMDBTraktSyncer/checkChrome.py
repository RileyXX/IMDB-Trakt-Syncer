...
def grant_permissions(path: Path):
    ...
    Ensures directories are accessible (add execute permission) and files are executable
    where needed.
        ...
        for item in dirs + files:
            ...
            try:
                ...
                if is_windows:
                    ...
                    os.chmod(
                        item_path, item_path.stat().st_mode & ~stat.S_IREAD
                    )  # Remove read-only attribute
                else:
                    # For Linux/macOS, add write permissions
                    current_permissions = item_path.stat().st_mode
                    item_path.chmod(
                        current_permissions | 0o777
                    )  # Add write permission (u+w)

                    # Ensure directories are executable
                    if item_path.is_dir():
                        item_path.chmod(
                            current_permissions | 0o777
                        )  # Add execute permission (u+x) for directories
                    else:
                        # For files (including chromedriver), make sure they are
                        # executable
                        item_path.chmod(
                            current_permissions | 0o777
                        )  # Add execute permission (u+x, g+x, o+x) for files
...
def get_user_data_directory():
    ...
    )  # Assuming this function exists and returns a version string
    ...
    if not chrome_binary_directory:
        raise FileNotFoundError(
            f"No Chrome binary directory found under {version_directory}"
        )
...
def is_chrome_up_to_date(main_directory, current_version):
    ...
    if not chrome_dir.exists():
        # Chrome directory for version not found. Chrome not downloaded or not up to
        # date.
        return False
    ...
    platform_binary = {
        "win32": [
            ...
            "chrome.exe",
        ],  # Two possible filenames for Windows
        "win64": [
            "chrome-headless-shell.exe",
            "chrome.exe",
        ],  # Two possible filenames for Windows
        ...
        ],  # Two possible filenames for macOS
        "mac-x64": [
            "chrome-headless-shell",
            "Google Chrome for Testing",
        ],  # Two possible filenames for macOS
        "linux64": [
            "chrome-headless-shell",
            "chrome",
        ],  # Two possible filenames for Linux
    }

    platform_key = get_platform()
    binary_names = platform_binary.get(
        platform_key, ["chrome-headless-shell", "chrome"]
    )  # Default to both names
            ...
            if subfolder.is_dir():
                for binary_name in binary_names:
                    if binary_name == "Google Chrome for Testing":
                        # For macOS regular Chrome, the binary is inside the .app bundle
                        ...
                        # For macOS headless shell, the binary is directly under the
                        # version directory
...
def is_chromedriver_up_to_date(main_directory, current_version):
    ...
    if not chromedriver_dir.exists():
        # Chromedriver directory for version not found. Chrome not downloaded or not up
        # to date.
        return False
...
def download_and_extract_chrome(
    download_url, main_directory, version, max_wait_time=300, wait_interval=60
):
    ...
    try:
        ...
        if expected_file_size and actual_file_size != expected_file_size:
            raise RuntimeError(
                f" - Downloaded file size mismatch: expected {expected_file_size} bytes"
                f", got {actual_file_size} bytes"
            )
        ...
        raise RuntimeError(
            f" - Failed to download or extract Chrome version {version}: {e}"
        )
        ...
        try:
            ...
            print(
                f" - Permission denied when trying to delete {zip_path}. Ensure no "
...
def download_and_extract_chromedriver(
    ...
    try:
        ...
        raise RuntimeError(
            f" - Failed to download or extract Chromedriver version {version}: {e}"
        )
        ...
        try:
            ...
            if chromedriver_dir.exists():
                for item in chromedriver_dir.iterdir():
                    # Check if the item is a subfolder (assumed to be the one containing
                    # the binary files)
                    if item.is_dir():
                        subfolder = item
...
def remove_old_versions(main_directory, latest_version, browser_type):
    ...
    # Delete downloaded browser if not the correct type (chrome vs
    # chrome-headless-shell)
        ...
        for version_dir in chrome_dir.iterdir():
            if version_dir.is_dir():
                for sub_dir in version_dir.iterdir():
                    if sub_dir.is_dir():
                        # All platforms (including macOS) use the same path for
                        # chrome-headless-shell
                        headless_shell_path = (
                            sub_dir / "chrome-headless-shell"
                            if os.name != "nt"
                            else sub_dir / "chrome-headless-shell.exe"
                        )
                        if headless_shell_path.exists():
                            print(
                                f"chrome-headless-shell found in {headless_shell_path},"
                                f" but script is set to use regular Chrome. Removing "
                                f"entire contents of {chrome_dir}..."
                            )
                    ...
                    if sub_dir.is_dir():
                        ...
                        if os.name == "posix":  # macOS
                            ...
                            chrome_path = (
                                sub_dir / "chrome"
                                if os.name != "nt"
                                else sub_dir / "chrome.exe"
                            )

                        if chrome_path.exists():
                            print(
                                f"Chrome found in {chrome_path}, but script is set to "
                                "use chrome-headless-shell. Removing entire contents "
                                f"of {chrome_dir}..."
                            )
                            try_remove(version_dir)
                            return  # Exit the function after removal
...
def get_chrome_binary_path(main_directory):
    ...
    raise FileNotFoundError(
        f"No valid Chrome binary found for platform {platform_key} in {chrome_dir}"
    )
...
def get_chromedriver_binary_path(main_directory):
    ...
    if not chromedriver_dir.exists():
        raise FileNotFoundError(
            f"Chromedriver version {version} not found in {chromedriver_dir}"
        )
...
def clear_selenium_manager_cache():
    try:
        ...
        if not selenium_install_location:
            print(
                "Could not determine Selenium install location. Skipping cache clear."
            )
            return

        webdriver_common_path = os.path.join(
            selenium_install_location, "webdriver", "common"
        )
        ...
        if os_name == "windows":
            selenium_manager_path = os.path.join(
                webdriver_common_path, "windows", "selenium-manager.exe"
            )
        elif os_name == "linux":
            selenium_manager_path = os.path.join(
                webdriver_common_path, "linux", "selenium-manager"
            )
        elif os_name == "darwin":  # macOS
            selenium_manager_path = os.path.join(
                webdriver_common_path, "macos", "selenium-manager"
            )
        else:
        ...
        # Build the command
        command = (
            f"{selenium_manager_path} --clear-cache --browser chrome "
            "--driver chromedriver"
        )
...
def checkChrome():
    ...
    if is_chrome_up_to_date(
        main_directory, latest_version
    ) and is_chromedriver_up_to_date(main_directory, latest_version):
    ...
    download_and_extract_chromedriver(
        chromedriver_download_url, main_directory, latest_version
    )
