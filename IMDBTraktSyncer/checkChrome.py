...
def remove_old_versions(main_directory, latest_version):
    ...
    for version_dir in chrome_dir.iterdir():
        if version_dir.is_dir() and version_dir.name != latest_version:
            ...
            shutil.rmtree(version_dir, onexc=remove_readonly)
...
def checkChrome():
    ...
    download_url = None
    # Use ['downloads']['chrome'] for non-headless Chrome browser
    for entry in latest_version_data['downloads']['chrome-headless-shell']:
