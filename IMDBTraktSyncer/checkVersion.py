...
def get_installed_version():
    try:
        ...
        print(
            f"Error: Could not retrieve IMDBTraktSyncer version using '{sys.executable}"
            f" -m pip': {e}"
        )
...
def get_latest_version():
    try:
        with urllib.request.urlopen(
            "https://pypi.org/rss/project/imdbtraktsyncer/releases.xml"
        ) as response:
...
def checkVersion():
    ...
    if compare_versions(installed_version, latest_version):
        print(
            f"A new version of IMDBTraktSyncer is available: {latest_version} "
