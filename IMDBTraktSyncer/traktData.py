...
def get_trakt_comments(encoded_username):
    ...
    for page in range(1, int(total_pages) + 1):
        response = EH.make_trakt_request(
            f"https://api.trakt.tv/users/{encoded_username}/comments",
            params={"page": page},
        )
        json_data = json.loads(response.text)
        ...
        for comment in json_data:
            ...
            if comment_type == "movie":
                ...
                show_movie_or_episode_title = (
                    f"{show.get('title')}: {episode.get('title')}"
                )
...
def get_trakt_watch_history(encoded_username):
    ...
    response = EH.make_trakt_request(
        f"https://api.trakt.tv/users/{encoded_username}/history?limit=100"
    )
    json_data = json.loads(response.text)
    total_pages = response.headers.get("X-Pagination-Page-Count")
    ...
    for page in range(1, int(total_pages) + 1):
        response = EH.make_trakt_request(
            f"https://api.trakt.tv/users/{encoded_username}/history?extended=full",
    ...
    # Filter watched_shows for completed shows where 80% or more of the show has been
    # watched AND where the show's status is "ended" or "cancelled"
